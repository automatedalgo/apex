...
def parse_fut_symbols(fut_symbols_raw, venue):
    ...
    assert fut_symbols_raw["code"] == "200000"
    data = fut_symbols_raw["data"]
    ...
    for item in data:
        ...
        # if not is_open:
        #     continue
        quote_ccy = item["quoteCurrency"]
        base_ccy = item["baseCurrency"]
        native_symbol = item["symbol"]
        norm_base_ccy = get_normalised_currency(base_ccy);
        norm_quote_ccy = get_normalised_currency(quote_ccy);
        ...
        inst_id = f"{asset_id_root}.PF.KCN"
        ...
        refitem = AssetRefData(
            ...
            lotQty=item["lotSize"],
            status=item["status"],
            quoteAssetPrecision=currency_map[norm_quote_ccy]["precision"],
            baseAssetPrecision=currency_map[norm_base_ccy]["precision"],
            minQty=item["lotSize"],
            minNotional=float('nan'),
...
def main():
    ...
    fut_symbols = load_json(f"{tmp_dir}/kucoin_fut_symbols.json")
